...
- 저장된 녹음, 프로젝트 파이프라인, Inbox 항목, 휴지통을 정리하는 경우: [프로젝트와 Inbox](guide:workspace-projects-and-inbox)
- 완성본을 내보내거나 Storage Management, Cloud Sync (E2EE), Message Center, 진단 등의 설정 진입점을 찾는 경우: [내보내기와 설정](guide:export-and-settings)
...
5. 저장된 작업을 다시 열거나, 프로젝트 context를 전환하거나, 파이프라인을 설정하거나, 휴지통을 검토하려면 [프로젝트와 Inbox](guide:workspace-projects-and-inbox)를 사용합니다.
6. [내보내기와 설정](guide:export-and-settings)에서 마무리하고, Sona가 작업 진행률, 복구, 업데이트, 자동화 결과를 표시할 때는 헤더의 Message Center 및 알림 센터를 확인합니다.

`AI Summary`, `Live Caption`, `Voice Typing`, 종단간 암호화(E2EE) 클라우드 동기화(Android 연동 지원), speaker profiles, vocabulary tuning은 핵심 전사 흐름 주변의 보조 기능입니다. 대부분의 경우 먼저 기본 전사 흐름을 작동시킨 뒤 필요한 확장 페이지를 여는 편이 가장 쉽습니다.
...
- 일상 사용에서 중요한 지원 화면: `Dashboard`, `Diagnostics`, `Storage Management`, `Cloud Sync`(E2EE), `Automation`, 메시지 센터(Message Center)
