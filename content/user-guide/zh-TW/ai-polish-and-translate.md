...
4. 如果需要管理 `Auto-Polish`、`Auto-Polish Frequency`、`關鍵詞`、指令式 `潤飾模式`（`去冗易讀`、`逐字精準`、`書面規整`）與 `自訂上下文`，請進入 `進階設定`，更完整的集中說明見 [詞彙與進階設定](guide:vocabulary-and-advanced-settings)。
...
3. 選擇目標語言。
4. 點擊 `開始翻譯` 或 `重新翻譯`。
5. 翻譯完成後預設開啟雙語對照顯示。可使用 `顯示雙語` 或 `隱藏雙語` 控制切換，並可直接在編輯器中按兩下譯文進行行內編輯修正。
