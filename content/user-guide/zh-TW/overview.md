...
- 需要整理已儲存的錄音、專案專屬流水線、收件匣或垃圾桶內容： [專案中心與收件匣](guide:workspace-projects-and-inbox)
- 要匯出成品、查看儀表板，或找到端到端加密雲端同步、儲存管理、訊息中心、診斷與備份入口： [匯出與設定](guide:export-and-settings)
...
5. 如果您要重新打開已儲存內容、切換專案上下文、為專案設定流水線或檢索垃圾桶，就進入 [專案中心與收件匣](guide:workspace-projects-and-inbox)。
6. 最後在 [匯出與設定](guide:export-and-settings) 完成匯出或查看關鍵設定；如果 Sona 主動彈出復原、更新或自動化結果，也從頂部訊息中心與通知入口繼續處理。

`AI 摘要`、`即時字幕`、`語音輸入法`、端到端加密跨裝置雲端同步（支援 Android 行動端配對）、說話者檔案和 `詞彙/進階設定` 都屬於圍繞主流程展開的擴充能力。通常先把核心轉錄流程跑通，再視需要打開對應頁面即可。

## 這份指南重點覆蓋什麼

- 推薦的首次離線上手路徑
- `即時錄音` 與 `批次轉錄` 的適用區別
- 編輯、專案整理、翻譯、匯出這些步驟如何接成同一條工作流程
- 說話者校對、說話者檔案和版本快照分別應該放在編輯與交付流程的哪個位置
- 日常最值得先了解的支援面：`儀表板`、`診斷`、`儲存管理`、`雲端同步`（E2EE）、`自動化` 與頂部訊息中心，而不是把每個設定都展開成完整手冊
...
> Sona 的正常路徑其實很直接：先把本機轉錄設定好，再生成或重新打開轉錄、整理文字，然後視需要使用專案整理、潤飾、翻譯、摘要和匯出。
