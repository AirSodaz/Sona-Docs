...
- 要匯出成品、查看儀表板，或找到端到端加密雲端同步、儲存管理、任務中心、診斷與備份入口： [匯出與設定](guide:export-and-settings)
...
6. 最後在 [匯出與設定](guide:export-and-settings) 完成匯出或查看關鍵設定；如果 Sona 主動彈出復原、更新或自動化結果，也從頂部任務中心與通知入口繼續處理。
...
- 日常最值得先了解的支援面：`儀表板`、`診斷`、`儲存管理`、`雲端同步`（E2EE）、`自動化` 與頂部任務中心，而不是把每個設定都展開成完整手冊
