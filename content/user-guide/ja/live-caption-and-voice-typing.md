...
`Settings > Input Device > Keep Microphone Active` は既定でオフです。アイドル中もマイクをウォーム状態に保って Voice Typing の遅延を下げたい場合はオンにします。録音、ディクテーション、入力プレビューをしていない間は Sona にマイクを解放させたい場合は、オフのままにしてください。
