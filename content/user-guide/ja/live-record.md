...
- 録音中の下書きも含め、作業内容を Sona のプロジェクトセンターに残したい場合
...
- 録音中でも、Sona はそのセッションを Draft アイテムとして[プロジェクトと Inbox](guide:workspace-projects-and-inbox)に表示できます。
- 録音を停止すると、その Draft が完成した文字起こしとして保存されます。別の新規ファイルを重複して作るのではなく、同じアイテムが更新されます。
- 事前にプロジェクトを開いていない場合、保存されたアイテムは通常 Inbox に入ります。

## お役立ちメモ

- 既定では `Ctrl + Space` で Live Record を開始または停止できます。
