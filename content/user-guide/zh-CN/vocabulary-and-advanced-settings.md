...
- `设置 > 词汇`：划分为 `识别与纠偏`、`提示词与上下文`、`说话人档案` 三个子标签页，管理 `热词`、`文本替换`、润色模式提示词、摘要模板与 `说话人档案`
- `LLM 润色 > 高级设置`：管理 `Auto-Polish`、频率、关键词、指令式 `润色模式`（`去冗易读`、`逐字精准`、`书面规整`）与自定义上下文
...
- 目前这项能力适用于兼容的 sherpa-onnx Transducer 模型；llama.cpp Qwen3-ASR 预设不使用热词。
...
- 说话人档案并不意味着自动归因一定正确。你仍然可以在 [编辑与播放](guide:edit-and-playback) 中确认建议、绑定其他档案，或把分组恢复为匿名。

## 什么时候打开 `高级设置`

- 当你不只想手动点一次 `LLM 润色`，而是还要管理 `Auto-Polish` 时。
- 当你需要调 `Auto-Polish Frequency`、关键词、指令式润色模式或自定义上下文时。
