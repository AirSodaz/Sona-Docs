`sona-cli` 是 Sona 的无状态命令行转写 Host。它不会打开或管理 Sona 的 SQLite 应用数据库、History/Tag 工作区、同步状态或 Online LLM 任务。转录结果只写入 `stdout`，或写入命令明确指定的输出文件。
...
- 打包版本：使用同平台安装包产物中附带的 `sona-cli` 可执行文件。
- 源码版本：`cargo run -p sona-cli -- <command> ...`
...
CLI 有意排除 SQLite、History、Tag、应用备份/恢复、Sync 和 Online LLM。它不会创建或修改桌面应用数据目录。请使用 `export transcript` 以及 `stdout`/文件输出，把 CLI 与其他工具组合起来。
...
生成带注释的本地转写和本地 API server TOML 配置模板。
...
列出、下载或删除本地 ASR 预置模型。这些命令只操作选定的模型目录，不操作 SQLite 应用状态。
...
转写一个本地音频文件；使用本地 ASR 时也可输入视频。不提供 `--online-provider` 时，命令使用已安装的本地 Sherpa 预置模型。
...
批量在线转写支持 `volcengine-doubao`、`groq-whisper` 和 `mistral-voxtral`。
...
使用 `--api-key-env NAME` 指定其他环境变量。`--online-config FILE` 接受用于覆盖 endpoint/model 等非敏感配置的 JSON 对象；其中不得包含 `apiKey` 或 `api_key`。
...
麦克风默认使用 CPAL 输入设备；`--device` 必须与 `--list-input-devices` 返回的完整名称匹配。`--output-format` 支持 `text` 和 `ndjson`；`--output` 可写入最终的 `json`、`txt`、`srt`、`vtt` 或 `md` 快照；`--format` 必须同时提供 `--output`。Ctrl+C、`stdin` EOF 和 `--duration` 都会先 flush/stop 会话再退出。

在线凭据和非敏感配置规则与 `transcribe` 相同。在线流式使用本地模型或运行时参数会被拒绝。

### `serve`

启动共享的本地 HTTP API server。CLI server 仅提供本地 ASR：它不暴露 Online ASR 或 WebSocket 流式转写。Online ASR 请直接使用 `transcribe` 或 `transcribe-live`。
