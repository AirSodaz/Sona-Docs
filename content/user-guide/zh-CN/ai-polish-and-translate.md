...
4. 如果需要管理 `Auto-Polish`、`Auto-Polish Frequency`、`关键词`、指令式 `润色模式`（`去冗易读`、`逐字精准`、`书面规整`）与 `自定义上下文`，请进入 `高级设置`，更完整的集中说明见 [词汇与高级设置](guide:vocabulary-and-advanced-settings)。
...
3. 选择目标语言。
4. 点击 `开始翻译` 或 `重新翻译`。
5. 翻译完成后默认开启双语对照显示。可使用 `显示双语` 或 `隐藏双语` 控制切换，并可直接在编辑器中双击译文进行行内编辑修正。
...
- 只有当当前转录来自已保存的项目条目时，菜单里才会出现 `重新转录`。
