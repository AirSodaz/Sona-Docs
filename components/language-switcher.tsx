...
import { getLocaleMetadata, isHomeLocale, supportedLocales } from '@/lib/locales';
...
export function LanguageSwitcher() {
  ...
  const currentLang = getLocaleMetadata(
    isHomeLocale(currentLocale) ? currentLocale : 'en',
  );
          ...
          >
            <div className="flex flex-col gap-0.5" role="menu" aria-orientation="vertical">
              {supportedLocales.map((locale) => {
                const lang = getLocaleMetadata(locale);
